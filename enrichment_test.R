...
iJO1366_bridging_centrality <- read_file('iJO1366_bridging_centrality.txt')
sum(iJO1366_bridging_centrality[,3] == 'False')
#p-value you want is the probability of getting 100 or more white balls 
#in a sample of size 400 from an urn with 3000 white balls and 12000 black balls
...
overlap_pvals_matrix <- matrix(unlist(overlap_pvals), ncol = 7, byrow = TRUE, dimnames=list(strains, strains))

overlap_pvals <- unlist(lapply(overlap_pvals, function(x){ -log10(x)}))
overlap_pvals.df <- expand.grid(strain1 = strains, strain2 = strains)
overlap_pvals.df$pval <- overlap_pvals
